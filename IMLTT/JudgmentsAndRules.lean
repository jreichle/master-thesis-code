  ...
  inductive IsEqualTerm : Ctx → Tm → Tm → Tm → Prop where
    ...
    | iden_comp : IsType (((Γ ⬝ A) ⬝ A) ⬝ (Tm.iden A a b)) C
                  → HasType (Γ ⬝ A) t (substitute (substitute (substitute C 0 2) 0 1)
                      (Tm.refl A 0) 0)
                  → IsEqualTerm (Γ ⬝ A) (Tm.j t 0 0 (Tm.refl A 0)) t
                      (substitute (substitute (substitute C 0 2) 0 1) (Tm.refl A 0) 0)
    | sigma_comp₁ : HasType Γ a A → HasType Γ b (substitute B a 0)
                    → IsEqualTerm Γ (Tm.prj₁ (Tm.pair a b)) a A
    | sigma_comp₂ : HasType Γ a A → HasType Γ b (substitute B a 0)
                    → IsEqualTerm Γ (Tm.prj₂ (Tm.pair a b)) b
                      (substitute B (Tm.prj₁ (Tm.pair a b)) 0)
end
