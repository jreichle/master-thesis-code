...
mutual
  ...
  inductive HasType : Ctx → Tm → Tm → Prop where
    ...
    | iden_elim : IsType (((Γ ⬝ A) ⬝ (lift 0 1 A)) ⬝ (Tm.iden A a b)) C
                  ...
                  → HasType (((Γ ⬝ A) ⬝ (lift 0 1 A)) ⬝ (Tm.iden A 2 1)) (Tm.j t 2 1 0) C
    -- conversion
  ...
  inductive IsEqualType : Ctx → Tm → Tm → Prop where
    ...
    | iden_form_eq : IsEqualType Γ A A' → IsEqualType (Γ ⬝ A ⬝ (lift 0 1 A)) (Tm.iden A 1 0) (Tm.iden A 1 0)
...
example (hA : IsType (Ctx.empty ⬝ 𝟙 ⬝ 𝟘 ⬝ 𝟙) (Tm.pi 2 1))
  ...
  HasType (Ctx.empty ⬝ 𝟙 ⬝ 𝟘) (Tm.indUnit Tm.tt a) (Tm.pi 2 1) :=
    HasType.unit_elim hA haA hbU
